...
<head>
    ...
    <title>Dashboard/Ndrysho Student-Profesore</title>
...
<body>
        ...
        if(isset($_GET['id'])){
            ...
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                require "../functions/ndryshoS.php";
            }

            $query = mysqli_query($connect, "SELECT * FROM perdoruesi WHERE userID = '$id';");
            $elem = mysqli_fetch_assoc($query);
            $emri = $elem['emri'];
            $mbiemri = $elem['mbiemri'];
            $email = $elem['email'];
            $user = $elem['userName'];
        }
        else{
        ...
        <div id="shtoMaini" >
            <div class="shtoTitle">
                ...
                <h6>Ketu mund te ndryshoni profesore ose studente !</h6>
            </div>
            <?php
            ...
            <form action="" class="forma" method="POST">
                <div class="first-row">
                    <div class="one">
                        <input type="text" name="sName" id="emri"  required>
                        <label for="emri"><?php echo $emri;?></label>
                    </div>
                    <div class="one">
                        <input type="text" name="sMbiemri" id="mbiemri" required>
                        <label for="mbiemri"><?php echo $mbiemri;?></label>
                    </div>
                </div>
                <div class="next">
                ...
                <div class="next">
                    <input type="text" name="sUser" id="user" required>
                    <label for="user"><?php echo $user;?></label>
                </div>
                <div class="next">
                    <input type="password" name="sPass" id="pass" required>
                    <label for="pass">Password</label>
                </div>
                <div class="next">
                    <select name="ssSelect" id="select">
                ...
                <div class="nextbtn">
                    <input type="Submit" name="sSubmit1" id="sub" value="Perfundo">
                </div>
            </form>
        </div>
